...
interface RelationGraphData {
  graph: string; // HTML string
}

const fetchRelationGraph = async (): Promise<RelationGraphData> => {
...
const RelationMap: FC<RelationMapProps> = () => {
  ...
  if (!data) {
    return (
      <div role="status" className="flex items-center justify-center pt-60">
        <svg
          aria-hidden="true"
  ...
  return (
    <div className="flex flex-col gap-6">
      <div dangerouslySetInnerHTML={{ __html: data.graph }} />
      {/* <iframe src="http://localhost:3000/api/relationMap"></iframe> */}
    </div>
  );
};
